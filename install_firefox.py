...
def step_install_rsat():
    ...
    warn("Se compare una finestra UAC, conferma per procedere.")
    log("Timeout per ogni comando: 10 minuti.")

    # Esegue i comandi direttamente in PowerShell elevato tramite
    # un unico processo powershell con -Command, senza Start-Process annidato
    # che causa il blocco silenzioso
    ps_commands = [
        (
            'Add-WindowsCapability -Online -Name "Rsat.ActiveDirectory.DS-LDS.Tools~~~~0.0.1.0"',
            "RSAT AD DS e LDS Tools (Add-WindowsCapability)"
        ),
        (
            "Install-WindowsFeature -Name RSAT-AD-PowerShell",
            "RSAT AD PowerShell module (Install-WindowsFeature)"
        ),
    ]
    ...
    for ps_cmd, description in ps_commands:
        ...
        try:
            result = subprocess.run(
                ...
                stderr=subprocess.STDOUT,
                text=True,
                timeout=600  # 10 minuti — RSAT può essere lento
            )
